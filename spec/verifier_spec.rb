...
RSpec.describe Verifier do
  subject(:verifier) { described_class.new(converter) }
  let(:converter) { instance_double('Converter') }

  before(:each) do
    allow(converter).to receive(:delete_hyphens)
  end
  ...
  describe '#isbn_10' do
    ...
    context 'when input is empty' do
      it 'raises an error' do
        not_10_digits = ''
        allow(converter).to receive(:isbn).and_return(not_10_digits)
    ...
    context 'when input is 10 digits long' do
      it 'doesnt raise an error' do
        ...
        allow(converter).to receive(:isbn).and_return(ten_digits)
    ...
    context 'when user has a valid ISBN code' do
      it 'notifies the user they have a valid ISBN code' do
        isbn = '3598215088'
        allow(converter).to receive(:isbn).and_return(isbn)
    ...
    context 'when user has an ISBN code which ends in X' do
      it 'processes the X as a 10' do
        ...
        deleted_x = '359821507'
        allow(converter).to receive(:isbn).and_return(isbn)
        allow(converter).to receive(:delete_x).and_return(deleted_x)
        expect { verifier.isbn_10(string) }.to output("You have a valid ISBN code\n").to_stdout
      end
    end
